...
所有对象 Schema 均为 strict；金额是安全整数，sequence/revision 为 uint64 十进制字符串，协议版本固定为 v4（完整视图必填公开盲注座位，见 [ADR-0003](../../docs/adr/0003-tex-53-authoritative-blind-seats.md)）。此包不依赖数据库、网络框架、UI 或 `poker-engine`，也不裁决扑克动作。权威 wire 语义见 [协议规格](../../docs/02-protocol-spec.md)。
