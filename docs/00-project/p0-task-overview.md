...
TEX-10 仓库治理基线完成
  ↓
TEX-11 工程地基 → TEX-12 测试地基
  ↓
TEX-13 / 14 / 15 扑克引擎 → TEX-16 引擎回归与模拟
  ↓
TEX-17 协议 + TEX-18 数据库
  ↓
TEX-19 房间大厅 → TEX-20 比赛运行时 → TEX-21 实时重连 → TEX-22 历史与监控
  ↓
TEX-23 Web 基础 → TEX-24 Lobby 页面 → TEX-25 牌桌 → TEX-26 / 27 体验与赛果
  ↓
TEX-28 联调安全 → TEX-29 压测监控 → TEX-30 发布验收
```
