...
TEX-21 在 `apps/{web,game-server}/src/{protocol,state,realtime}/**/*.test.ts` 使用可注入 Fake WebSocket、UUID、Event Bus 与 Fake Clock 验证认证超时、版本/Token 拒绝、15/45 心跳、旧会话接管、投影乱序、Token 降级与重连重试；它们属于 Unit 层，不替代后续 `tests/clients/` 的多客户端压力/故障注入测试。
