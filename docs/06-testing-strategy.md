...
> TEX-26 增加 Fake Clock 驱动的 AnimationQueue 与 AudioAdapter 回归：连续/重复 Event 的顺序与 canonical/presentation 分离、Snapshot/重连/Hard Forward 清队列、取消/异常/Reduced Motion 终帧、Burn 无牌面、服务端 `bestFiveCards` 公开投影、Soft/Hard catch-up 不影响 LegalActions/Clock/命令、音效开关和 autoplay/播放失败降级。Transport/Projection 的 Fake WebSocket 测试覆盖受限 Snapshot 请求、`SESSION_REPLACED` 冻结和单连接重试；不依赖真实网络、game-server 或 sleep。
