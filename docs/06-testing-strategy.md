...
### 9.2 TEX-38 动画与音效日常回归

前端展示优化的独立回归位于 `tests/e2e/animation-audio/`：使用经过现有协议 Schema 的 WS 投影夹具，验证真实浏览器的动效、分池/牌型、设置持久化、后台恢复和低性能降级；它不替代 §6 的真实服务端/数据库联调。队列、慢帧检测、媒体生命周期与受限存储使用注入时钟/适配器的单元测试，断言取消后无残留 timer 或旧 cue。

TEX-38 本地运行使用独立端口（`TEX_E2E_PORT=3138`）和单 worker，避免与 TEX-29 压测争用端口；性能结论须记录浏览器、视口、CPU 限速和代码版本。模拟设备与受限 CPU 的结果仅为日常回归，§9.1 的 Android/iPhone 发布实机验收仍单独执行。证据和复现命令见 [TEX-38 验收记录](./03-engineering/TEX-38-acceptance.md)。
...
Release artifact CI 落地事实（2026-09-08，TEX-40）：`.github/workflows/release-artifact.yml` 只在**不可变提交**的干净 checkout 上运行——push/PR 用 `github.sha`（不用可移动分支 ref），`workflow_dispatch` 的 `inputs.sha` checkout 前校验为 40 位十六进制并断言 `git rev-parse HEAD` 一致；先跑质量门禁（lint/typecheck/build/unit/rules/integration/ws/release + Simulator smoke，绑定实际 SHA），再以 `infra/deployment/scripts/build-artifact.mjs` 构建 Game Server 最小运行 artifact（编译产物 + 生产 node_modules + 版本化迁移 SQL/journal，无 monorepo 源码/devDeps/Secret；构建前拒绝脏工作树并 `--expect-git-sha`），生成 `tarballSha256`/`rootDigestSha256`/关键文件 checksum 的 manifest；`verify-artifact.mjs` 解压到隔离目录复核摘要后，由 `artifact-smoke.mjs` 在独立 postgres service 的临时 schema 上以 artifact 自身 migrator 迁移、以生产启动命令（`NODE_ENV=production`）启动并断言 `/health` 与 WS Upgrade/协议关闭（非法帧→`INVALID_MESSAGE`、未认证 5s→关闭码 4003）。**正式 artifact 仅当 build + verify + smoke 全部成功才上传**（失败诊断以 `-DIAG-NOT-PUBLISHABLE` 命名）。受控发布/回滚编排（原子 `current` 运行目标 + 失败回切）与运行手册见 `infra/deployment/README.md` 与 `docs/05-operations/README.md`（含 TEX-41/42/43/39 边界）。
