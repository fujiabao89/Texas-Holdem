...
> **实现核对（TEX-46）**：操作区（`.table-action-dock`）是否渲染由 `table-state.ts` 的准入决定——本人是当前行动者、`viewer.legalActions` 非空、连接为 `CONNECTED`、无 pending 命令、未处于同步禁用。它锚定在牌桌容器为行动区固定预留的高度带内，因此等待、观战、淘汰、断线与非本人回合都不渲染任何可提交控件。提交后 pending 期间准入立即为假，操作区随即收起；牌桌尺寸不变。手机尺寸下金额面板固定三行（快捷额、金额滑杆与 ±、返回/提交），展开精确输入时以输入框替换快捷额行而不增高。
