...
| --- | --- | --- |
| `HAND_STARTED` / `DEAL_HOLE_CARD` | 桌面始终呈现一个位于公共牌下方且不遮挡 Board 层的可见 Deck；每个接收者都对同一已投影 Event 看见牌背从该 Deck 沿连续的上扬弧线飞向目标座位手牌区。自己的牌到位、停顿后才翻开，其他人始终保持牌背。飞行只使用合成层 `transform` / `opacity`，不得仅播放音效或用通用提示替代可见发牌 | 《区块1-5 v0.1》§5.5 |
