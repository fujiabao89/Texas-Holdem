...
- 2026-09-09 用户确认 River & Raise 产品首页：暖白底色、深绿文字和桌布、陶橙主按钮；首页包含品牌主视觉、玩法引导与创建/加入入口。原两个核心功能保持不变。
- “开始一局”以原生 dialog 展示创建/加入选择，已有邀请码直接进入 `/join`；`/create`、`/join?code=…` 与 Room 路由保持兼容。首页装饰牌不读取或表示任何真实牌局。
- 全站复用品牌导航、控件、表单与状态反馈。页面入场 300ms、按钮反馈约 180ms、历史面板入场 240ms；位移/入场使用 transform/opacity，按钮另有颜色与阴影反馈，不等待动效完成才导航或提交。路由外 Provider 保持挂载，动效不参与游戏队列与权威状态。
- 系统减少动态效果或既有设备 motion=reduce 时：首页倾斜/展开、页面/弹层/抽屉位移与按钮位移关闭；仍呈现完整内容和静态 focus/hover 状态。仅牌桌保留既有事件队列与降级契约。
...
- 极简、明亮、白色主体；牌桌可使用深青绿色椭圆毛毡面与深色桌沿作为唯一强色块，页面留白与状态条保持白色，操作面板保持浅色且可使用低透明度玻璃材质；**不赌场化**（无霓虹、金币、VIP、宝箱或高饱和装饰）。
- TEX-25 桌面视觉顺序固定为：顶部轻量状态条、中央椭圆牌桌（底池与公共牌居中、玩家环绕）、底部本人 Seat 与悬浮操作区。深青绿色仅表达牌桌实体，不能扩展为整页深色背景。

- TEX-45（2026-09-11）：延续 River & Raise 暖白/深绿风格，桌面牌桌最大宽度 1060px，细纹桌布与桌沿层次仅使用静态 CSS。中央将当前街、底池标签与等宽金额分层；顶部显示 canonical 盲注；本人 Seat 使用暖白面板，行动者保留高亮，筹码采用千位分组。标准牌值/花色与牌点保留，增强两端牌角和牌背纹样。640–1000px 使用较高椭圆（640–760px 进一步拉开左右座位）避免拥挤，手机仍允许纵向滚动。下注面板显示行动提示，延时作为辅助行；不改变合法动作、现有牌尺寸、发牌/翻牌/派奖时序、动画关键帧或音效。

- TEX-46（2026-09-16）：牌桌页收敛为单一视口，取代 TEX-25/TEX-45 记录的“手机允许纵向滚动至操作区”。站点页头之外的剩余高度归页面所有，牌桌在剩余空间内按 `--felt-ratio` 自适应，并固定预留 `--dock-reserve` 给操作区；操作区因此只出现在本人 Seat 前方的牌桌下沿，且出现与消失都不改变牌桌几何。合并 TEX-47 的稳定顺时针槽位后，3 人以上统一启用压缩与纵向分层，以覆盖连续和稀疏物理座位映射；本手结果与终局排名改为浮于牌桌之上的覆盖层。牌面尺寸、Deck/Board 几何、发牌/翻牌/派奖时序、动画关键帧与音效不变。
- TEX-46 顶栏：品牌标识靠左，牌桌标题与服务端盲注居中，牌局记录、音效开关与连接状态靠右；设置入口仍可用。牌桌内容区不再另设标题/操作白色面板；窄屏在同一顶栏内换行，并继续满足单视口验收。
- TEX-46 行动区：采用受 iPhone Liquid Glass 启发的悬浮玻璃操作条、双层高光边缘与胶囊按钮，保留高对比文字和语义色；主提交按钮使用绿色高光渐变，全下二次确认使用深暖色。手机主操作排为两列，延时入口单独占末行。桌面下注展开态为两行并列排布，含快捷金额、滑杆与 ±、返回、精确输入、全下和提交；手机为三行。快捷比例与金额分层，已选快捷额具备视觉和辅助技术状态，当前金额与滑杆进度同步显示。各尺寸的展开态不得需要页面或面板内滚动；不支持背景模糊、或用户要求降低透明度时使用不透明浅色回退。
- TEX-46 中央留白与整体布局调整：当前街、底池总额与行动者集中在公共牌上方，手机竖屏分两行。单底池只显示总额，保留同一节点的派奖定位与高亮；有边池时继续展示各池明细。公共牌与本人手牌分层，不以装饰水印占据间隔。本人手牌正常桌面 88px 高、矮桌面/平板 76px、手机竖屏 66px、短横屏 60px；对手手牌桌面 42px、手机 28px，所有发牌槽与翻面组件同步缩放。人数模板及桌沿布局按 §7.2，取代早期统一压缩分层和原牌尺寸约束；不改变牌局规则、DOM 实测定位方式或动画时序。

- TEX-46 小牌可读性：对手公开底牌、摊牌候选/最佳五张与本手结算的小尺寸牌面使用单组牌值与花色，分行且随牌宽缩放，不与两端牌角或中央图案叠加。公共牌与本人手牌保留标准牌点样式，牌角和中央图案字号同样随各自牌宽缩放；所有牌的可访问名称与服务端投影不变。
...
- **自身 Seat 无论真实编号，视觉上始终位于屏幕下方偏中央**（《总规划》§7.1）。
- TEX-46 整体布局（2026-09-23，用户批准）：取代 TEX-47 的统一十槽位偏移公式。按 Snapshot 完整参赛名单选择 2 人面对面、3–6 人六人环、7–10 人十人环模板；在模板内按相对本人 `seatIndex` 的顺时针顺序填充，本人仍固定下方中央。弃牌、全下、淘汰、撤回只改变状态，不过滤参赛名单，因此行动者、筹码、状态、昵称或重渲染均不引起换位；名单真正变化或新比赛时允许重新选择模板。稀疏物理座位与连续座位采用同一规则，不改变服务端座位编号。
- 对手玩家卡与牌背较旧桌面缩小约 30–40%，本人手牌只略缩并始终大于对手。座位跨在桌沿，当前街下注额独立放在对应座位朝桌心的一侧，具备玩家名称和金额的无障碍描述；不得以下注数值变化改变座位高度或坐标。当前行动高亮、身份徽标与公开/私有牌权限不变。
- Dealer Button、SB/BB 徽标随每手位置移动，但 Seat 视觉位置不变（[01](./01-engine-spec.md) §11）。三个标识只消费服务端 `dealerSeat/smallBlindSeat/bigBlindSeat` 投影（[02](./02-protocol-spec.md) §9.2；TEX-53）；Heads-Up 的 D=SB 在同一 Seat 同时显示，客户端不按人数、筹码或存活状态推断。
...
- 当前行动者清晰高亮 + 倒计时（§11.1）；高亮只改变视觉强调，不移动 Seat，也不得遮挡 D/SB/BB、昵称、筹码或手牌。Fold 后牌背/玩家卡降低视觉权重；All-in 明确显示 `ALL-IN`，不能与淘汰混淆；断线玩家显示 `Disconnected`（《区块1-5 v0.1》§5.10）。
...
- Mobile 对局优先横屏：宽度 ≥640px、高度 ≤500px 的短横屏压缩顶栏，牌桌在左、完整行动区在右，时钟与提示位于右侧操作列下方；不强制旋转、不锁定方向。竖屏保留可完成对局的单视口回退，并提供非阻断横屏建议。浏览器矩阵补充 844×390、800×360 横屏，实机方向切换仍需发布验收。
...
- TEX-46：以上每个尺寸都是**单视口**验收点——轮到本人行动时，页面不出现纵向或横向滚动，牌桌、本人手牌、公共牌、底池、行动状态与全部合法操作（含手机金额面板的快捷额、Slider、±、精确输入与 All-in 两步）都在同一视口内可达。
...
- 操作区通常位于**本人 Seat 前方的牌桌下沿**；短横屏手机使用桌面右侧的固定操作列（§7.5）。不得遮挡本人手牌、公共牌、底池、任何 Seat 或关键动画；其出现与消失不得改变牌桌几何（§7.1 TEX-46）。

> **实现核对（TEX-46）**：操作区（`.table-action-dock`）是否渲染由 `table-state.ts` 的准入决定——本人是当前行动者、`viewer.legalActions` 非空、连接为 `CONNECTED`、无 pending 命令、未处于同步禁用。它锚定在牌桌容器为行动区固定预留的高度带或短横屏右侧列内，因此等待、观战、淘汰、断线与非本人回合都不渲染任何可提交控件。提交后 pending 期间准入立即为假，操作区随即收起；牌桌尺寸不变。桌面金额面板固定两行、手机固定三行；展开精确输入时以输入框替换快捷额而不增高。
...
### 9.7 TEX-38 体验优化契约（2026-09-05）

- 保留 §9.6 已确认的发牌/翻牌/Showdown 节奏。每个展示任务以 `tournamentId + handId + sequence` 标识实例，组件消费队列提供的实际时长，普通动作的 CSS 与 Soft Catch-up 同步。
- 下注、跟注、加注、盲注与 All-in 在对应 Seat 展示动作/金额并向 Pot 移筹；Check 为短双击反馈，Fold 只移出牌背；Burn 从可见 Deck 移入弃牌区域且永不制造牌面。未跟注退回和每个 `POT_AWARDED.awards` 的金额、方向及赢家均只使用服务端字段。
- Reveal 展示服务端牌型与最佳五张牌；静态本手结果按已完成展示事件累积公开 Reveal 与每 Pot 分配，减少动态效果时直接提交该摘要。摘要不重算牌型、不跨手保留，Snapshot/重连清空旧摘要，可通过历史入口查看归档结果。
- 减少动态效果由系统偏好或用户主动选择触发，只取消位移/翻转，不强制静音。连续慢帧触发本次牌桌会话的同等降级；后台不采样性能、不播放声音、不积累动画。返回前台清理旧展示并使用既有 Snapshot 请求验证最新状态。
- 发牌的入队规划与当前正在播放的手牌进度分别维护；下一手提前入队不能清空上一手正在发的牌。重复或过期展示事件不重播；音频回调异常不缩短视觉任务。
- Hard Forward 的状态可见至少 120ms，并提供 3 秒同步提示；若跳过公开摊牌，先以已接收的服务端结果保留至少 1 秒静态摘要。所有屏障与卸载取消旧 timer、overlay 和声音。
- 高频发牌不逐张触发读屏播报；当前行动提示读取最新 canonical，公开结果使用静态可访问结构。动画仅影响展示，不遮挡下注区或改变命令输入。
...
- 浏览器自动播放策略：当前以复用的 `HTMLAudioElement` 在首次用户交互后解锁；播放失败静默降级，不影响牌局（生命周期见 §10.3）。
...
### 10.3 TEX-38 偏好与生命周期（2026-09-05）

- 牌桌与设置共用设备级偏好来源：音效开关、0–100% 音量（默认 80%）、动态效果（跟随系统/减少）。兼容既有 `texas-holdem:sound-enabled`；SSR 首帧安全，存储受限时使用内存降级，同页与跨标签页更新一致。偏好不包含身份或牌局数据。
- 保留 §10.2 已选定音色和独占通道；主音量按 cue 相对响度应用，音量归零与关闭开关立即取消当前及延迟声音。减少动画下的当前事件使用即时 cue，不补播之前跳过的声音。
- Pointer 与键盘用户手势均可解锁；成功解锁幂等，失败允许后续手势重试。关键声音仅在进入牌桌后的空闲时段预取，其余按需复用本地元素。播放、预取、解锁失败均静默降级。
- 卸载释放媒体元素、事件 listener 和延迟任务；React Strict Mode 重挂载可重新启用控制器，异步旧回调不能恢复播放。设置提供有名称的开关、原生可键盘操作的音量 Slider 及 Kenney 署名。
- 解锁与播放均设 5 秒清理期限，避免挂起媒体占用通道；播放时若浏览器重新拒绝自动播放，只允许下一次手势重新解锁。普通同类 cue 的 90ms 连续重复可合并，关键发牌/全下/派奖不可丢失。
- 本人行动机会和盲注升级提醒只观察已接受的 canonical 状态变化，Snapshot/重连/隐藏重置基线；低优先级提醒最多等待 900ms，不抢占关键 cue 或其他控制器。顺子及以上使用服务端 `winningHandRank.category` 对既有派奖素材作克制的速率/音量变化，始终配合中文公开结果，不引入前端评牌。
...
| 响应式 | ~390×844、~360×800、平板、1366×768、1920×1080；10 人桌姓名/Stack/D/SB/BB/当前 Actor/操作区不重叠（《区块6-10 v0.2》§9.18）。TEX-46 起另需在这些视口中以单视口完成行动：无需纵向/横向页面滚动，操作区仅在自己行动时出现且不改变牌桌几何 |
...
- **为什么自己的 Seat 固定在下方中央**：无论真实座位号，视觉上第一时间定位自己与自己的底牌；其他座位在人数模板内保留相对 `seatIndex` 的顺时针顺序，行动与状态变化不重排，D/SB/BB 始终绑定真实座位（§7.2）。
...
- **为什么操作区固定预留高度带**：行动区出现与消失若改变牌桌几何，每次行动权切换都会造成布局跳动，玩家需要重新定位手牌与公共牌。预留带让牌桌尺寸在一手牌内恒定，代价是不行动时牌桌下方留白（TEX-46）。
