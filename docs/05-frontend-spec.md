...
- TEX-46 中央留白调整：当前街、底池总额与行动者集中在公共牌上方，手机分两行。单底池只显示总额，保留同一节点的派奖定位与高亮；有边池时继续展示各池明细。公共牌与本人手牌分层，不以装饰水印占据间隔。为适应短视口，本人手牌在高度 ≤820px 的桌面/平板缩为 80px 高（正常桌面 96px、手机 72px），所有发牌槽与翻面组件同步缩放；这项调整取代此前保持原牌尺寸的视觉约束，不改变牌局规则、座位相对映射、DOM 实测定位方式或动画时序。
...
- 座位视觉映射只由服务端 `seatIndex` 决定：本人固定在下方中央槽位，其他 Seat 按相对本人 `seatIndex` 的顺时针偏移映射到固定槽位（TEX-47）。同一桌内该映射不随入座人数、当前行动者、筹码、连接状态、弃牌、全下、淘汰、撤回 Seat、昵称长度或组件重渲染而改变；空 Seat 不渲染玩家卡，只保留无障碍语义，不以普通文档流占位破坏椭圆牌桌。
...
- 操作区位于**本人 Seat 前方的牌桌下沿**，不得遮挡本人手牌、公共牌、底池、任何 Seat 或关键动画；其出现与消失不得改变牌桌几何（§7.1 TEX-46）。

> **实现核对（TEX-46）**：操作区（`.table-action-dock`）是否渲染由 `table-state.ts` 的准入决定——本人是当前行动者、`viewer.legalActions` 非空、连接为 `CONNECTED`、无 pending 命令、未处于同步禁用。它锚定在牌桌容器为行动区固定预留的高度带内，因此等待、观战、淘汰、断线与非本人回合都不渲染任何可提交控件。提交后 pending 期间准入立即为假，操作区随即收起；牌桌尺寸不变。桌面金额面板固定两行、手机固定三行；展开精确输入时以输入框替换快捷额而不增高。
...
- **为什么自己的 Seat 固定在下方中央**：无论真实座位号，视觉上第一时间定位自己与自己的底牌；其他座位按 `seatIndex` 相对本人顺时针固定映射（TEX-47），同一桌内不因人数或状态变化重排，行动方向、D/SB/BB 与座位一一对应（《总规划》§7.1）。
