...
> **2026-08-26 增量核对（TEX-26 精修）**：牌桌的公共牌按已连续应用的 Event 在目标牌框中逐张“入框 → 翻面”；Showdown 只将服务端 `bestFiveCards` 在已公开七张候选牌中作视觉组合，未选牌淡出，客户端不重算规则。Kenney 本地音效使用预加载 voice pool，并将一条 Flop Event 的三个 cue 对齐三张牌到位；相关验收与 Fake Clock 测试见 [05](./05-frontend-spec.md) §9–§10、[06](./06-testing-strategy.md)。
