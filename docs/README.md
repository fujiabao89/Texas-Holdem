...
> 项目现状（2026-08-24 核对）：TEX-11 已建立 pnpm monorepo 工程地基；TEX-13～15 已实现扑克 Engine 子域；TEX-17 已实现协议包的严格 Schema、推导类型、消息/错误信封及纯投影；TEX-18 已实现持久化层；TEX-23 已实现 Web 路由壳、中文文案、客户端投影状态和 HTTP/WS Transport 基础（不含业务页面）。服务端 Room/Lobby 低频 HTTP 运行时（TEX-19：§4/§5/§10）已实现；Tournament 运行时与前端业务逻辑仍未实现。规格 05 的 §4/§5、§14.1、§15 与 §17 中与上述基础边界对应的部分已核对实现；其余章节及 04/06 中未实现部分仍为**设计意图**。
