...
> **TEX-53 实际存储核对**：生产 Commit Bundle 将 `snapshot.state` 作为 canonical JSON 字符串写入 jsonb，数据库读取仍可能返回字符串；Drizzle jsonb 映射对字符串执行解析，因此 RecoveryRepository 返回对象，恢复编排对该对象复算 checksum。TEX-53 以真实 Bundle → PostgreSQL → Repository → 恢复验证这一路径，未修改读取逻辑。本次保持持久化 `schemaVersion=2`、`engineVersion=0.1.0`，与 wire v4 / wire `snapshotVersion=1` 分别管理，无迁移。
