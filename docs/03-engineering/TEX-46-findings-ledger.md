...
- **F-02 的矮桌面上限**：1366×768 无法在不把牌桌压缩到 Seat 相交的前提下，把 `--dock-reserve` 从 104px 提高到展开编辑器所需的约 203px；该视口展开金额面板时仍可能需要内部滚动。768×1024 已通过通用三行编辑器、编辑器内全下入口与 208px 预留带解决，并由展开态回归覆盖。1366×768 的残留未在本次冲突合并中扩大处理范围。
