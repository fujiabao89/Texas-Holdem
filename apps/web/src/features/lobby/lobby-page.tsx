...
export function LobbyPage({ roomId }: { readonly roomId: string }) {
  ...
  return <LobbyFrame>
    <div className="rr-lobby-header flex flex-wrap items-center justify-between gap-3"><h1 className="text-3xl font-bold">{message("room.lobbyTitle")}</h1><p aria-live="polite" className="rr-status">{connectionState === "CONNECTED" ? message("room.connected") : connectionState === "CLOSED" ? message("room.disconnected") : message("room.connecting")}</p></div>
    ...
    <section className="rr-panel rr-lobby-seats grid gap-4" aria-labelledby="seats-heading"><h2 id="seats-heading" className="text-xl font-semibold">{message("room.seats")}</h2><div className="grid grid-cols-1 gap-2 sm:grid-cols-2">{seats.map((player, seat) => <div className={`lobby-seat-feedback flex min-h-20 items-center justify-between rounded-xl border p-3 ${player?.ready ? "border-emerald-200 bg-emerald-50" : "border-neutral-200 bg-white"} ${player === null ? "rr-seat-empty" : ""}`} key={`${seat}:${player?.playerId ?? "empty"}:${player?.ready ?? false}`}><span className="rr-seat-number" aria-hidden="true">{seat + 1}</span>{player === null ? <><span>{message("room.emptySeat")}</span>{!locked && <button className="text-sm underline underline-offset-4" disabled={pending} onClick={() => void update({ type: "CHANGE_SEAT", seat })}>{message("room.selectSeat")}</button>}</> : <div><p>{player.displayName}{player.playerId === room.hostPlayerId ? ` · ${message("room.host")}` : ""}</p><p className="text-sm text-neutral-600">{player.ready ? message("room.readyState") : message("room.notReady")}{player.connectionStatus === "DISCONNECTED" ? ` · ${message("room.disconnectedMember")}` : ""}</p></div>}</div>)}</div>
      {!locked && <div className="flex flex-wrap gap-2"><button className="rr-button" disabled={pending || seats.every((player) => player !== null)} onClick={() => void update({ type: "CHANGE_SEAT", seat: seats.findIndex((player) => player === null) })}>{message("room.randomSeat")}</button>{self?.seat !== null && <button className="rr-button" disabled={pending} onClick={() => void update({ type: "CHANGE_SEAT", seat: null })}>{message("room.leaveSeat")}</button>}<button className="rr-button" disabled={pending || connectionState !== "CONNECTED"} onClick={setReady}>{self?.ready ? message("room.unready") : message("room.ready")}</button></div>}
    </section>
    <section className="rr-panel grid gap-2" aria-labelledby="invite-heading"><h2 id="invite-heading" className="text-xl font-semibold">{message("room.invite")}</h2><p className="rr-invite-code">{room.inviteCode ?? message("room.closed")}</p><div className="flex flex-wrap gap-2">{room.inviteCode !== null && <><button className="rr-button" onClick={() => void copy(room.inviteCode as string)}>{message("room.copyCode")}</button><button className="rr-button" onClick={() => void copy(`${window.location.origin}/join?code=${room.inviteCode}`)}>{message("room.copyLink")}</button></>}</div></section>
    <section className="rr-panel" aria-labelledby="config-heading"><h2 id="config-heading" className="text-xl font-semibold">{message("room.roomConfig")}</h2><dl className="mt-2 grid grid-cols-2 gap-2 text-sm"><dt>{message("room.maxPlayers")}</dt><dd>{room.config.maxPlayers}</dd><dt>{message("room.startingStack")}</dt><dd>{room.config.startingStack}</dd><dt>{message("room.smallBlind")}</dt><dd>{room.config.smallBlind}</dd><dt>{message("room.bigBlind")}</dt><dd>{room.config.bigBlind}</dd></dl></section>
    {isHost && !locked && <section className="rr-panel grid gap-3" aria-labelledby="host-heading"><h2 id="host-heading" className="text-xl font-semibold">{message("room.hostControls")}</h2><HostConfigEditor room={room} onSave={(config) => void update({ type: "UPDATE_CONFIG", config })} disabled={pending} /><button className={primaryButton} disabled={!allReady || pending} onClick={() => void start()}>{message("room.start")}</button>{!allReady && <p className="text-sm text-neutral-600">{message("room.startNeedPlayers")}</p>}<div className="grid gap-2">{room.players.filter((player) => player.playerId !== room.hostPlayerId).map((player) => <button className="w-fit rounded border border-red-300 px-3 py-2 text-sm" key={player.playerId} disabled={pending} onClick={() => void update({ type: "KICK_PLAYER", targetPlayerId: player.playerId })}>{message("room.kick")}：{player.displayName}</button>)}</div></section>}
    {feedback !== null && <p className="rr-feedback" role="status">{feedback}</p>}
  </LobbyFrame>;
}

function LobbyFrame({ children }: { readonly children: ReactNode }) {
  const { motion } = usePresentationPreferences();
  return <main data-reduced-motion={motion === "reduce"} style={{ "--lobby-feedback-duration": `${visualTimings.lobbyFeedback}ms`, "--control-feedback-duration": `${visualTimings.controlFeedback}ms` } as CSSProperties} className="rr-page rr-lobby lobby-controls">{children}</main>;
}
...
function HostConfigEditor({ room, onSave, disabled }: { readonly room: RoomSnapshot; readonly onSave: (config: TournamentConfig) => void; readonly disabled: boolean }) {
  ...
  return <form className="grid grid-cols-2 gap-2" onSubmit={submit}>
    <label className="grid gap-1 text-sm">{message("room.maxPlayers")}<input className="rr-input" type="number" min={2} max={10} value={config.maxPlayers} onChange={(event) => setConfig((value) => ({ ...value, maxPlayers: Number(event.target.value) }))} /></label>
    <label className="grid gap-1 text-sm">{message("room.startingStack")}<input className="rr-input" type="number" min={1} value={config.startingStack} onChange={(event) => setConfig((value) => ({ ...value, startingStack: Number(event.target.value) }))} /></label>
    <label className="grid gap-1 text-sm">{message("room.smallBlind")}<input className="rr-input" type="number" min={1} value={config.smallBlind} onChange={(event) => setConfig((value) => updateInitialBlind(value, { smallBlind: Number(event.target.value) }))} /></label>
    <label className="grid gap-1 text-sm">{message("room.bigBlind")}<input className="rr-input" type="number" min={2} value={config.bigBlind} onChange={(event) => setConfig((value) => updateInitialBlind(value, { bigBlind: Number(event.target.value) }))} /></label>
    <button className="rr-button col-span-2" disabled={disabled} type="submit">{message("room.saveConfig")}</button>
  </form>;
}
