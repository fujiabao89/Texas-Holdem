...
`.rr-site:has(.rr-table-page)` 把站点页头之外的剩余高度交给页面，`#page-content` 改为 flex 填充且页面本身不滚动；牌桌容器以 `container-type: size` 按 `--felt-ratio` 在剩余空间内取最大可用尺寸。操作区预留 `--dock-reserve`（桌面 124px、手机竖屏 176px，短横屏改用右侧列），行动区（`.table-action-dock`）只在该预留区域内出现或消失，因此不改变牌桌几何、不产生布局跳动。操作区采用半透明模糊底材、双层高光边缘和胶囊玻璃按钮，主提交按钮使用绿色高光渐变，全下二次确认时转为深暖色。手机主操作以两列排列，延时入口存在时占满最后一行。浏览器不支持背景模糊或用户偏好降低透明度时回退为不透明浅色面板。桌面金额编辑器以两行并列展示快捷额、滑杆、返回、精确输入、全下与提交；手机保留三行排列，均不依赖面板内部滚动。快捷额分层显示比例与金额，通过 `aria-pressed` 表达选中状态；圆形滑杆手柄与填充轨道反映当前权威范围内的草稿金额。
...
座位围绕桌沿居中，信息卡跨坐桌边；对手卡片和手牌约缩小三分之一，自己的手牌保持明显更大。`streetBet` 从 Seat 中移到独立 `.table-bets` 层，带玩家名称的辅助标签与内侧筹码标记，不因下注清零而改变 Seat 高度。连续与稀疏物理座位共用人数模板；公共牌、底池与行动区互不遮挡。本手结果层（`HandOutcomeSummary`、终局排名与赛果入口）仍为浮层。牌局动画、命令信封、投影语义与音效未变。

回归见 `tests/e2e/table-layout/single-viewport.spec.ts`（视口 × 人数矩阵）。

中央信息区将当前街、底池总额与行动者集中在公共牌上方；手机竖屏分两行。仅有主池时不再重复显示下方的“主池”标签，总额节点同时保留 `data-pot-index=0` 作为派奖坐标与高亮目标；存在边池时仍显示各池明细。本人手牌正常桌面 88px 高、矮桌面 76px、手机竖屏 66px、短横屏 60px；对手分别使用 42px 或 28px 高手牌，发牌槽、牌背、翻面组件共享尺寸变量。动画仍读取实际 DOM 边界，时序不变。

手机优先横屏但不强制锁定：宽度至少 640px 且高度不超过 500px 的横屏使用左桌面、右侧 252px 操作列与紧凑顶栏；操作列永久预留，时钟和提示位于其下方。竖屏保留完整操作和非阻断横屏建议。舞台内边距同时为桌沿外溢卡片和操作区留位，不再用舞台最小高度撑出视口。浏览器验收覆盖公共牌/边池/手牌间距、独立下注、桌沿位置和全套下注控件，实机横竖屏切换仍需发布前验证。
