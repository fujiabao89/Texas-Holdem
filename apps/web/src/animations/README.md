...
手牌任务只使用已过滤的 `DEAL_HOLE_CARD`：牌背从 Deck 飞向 `seat` 对应手牌区，本人仅在 Event 已携带的 `card` 存在时于到位后翻面；绝不从 Snapshot 回填对手牌面。公共牌任务携带该 Event 的公开牌和动画前 Board 长度：Flop/Turn/River 在目标牌框中从 Deck 方向到位、再翻面；不为 Burn 生成牌面。`PLAYER_REVEALED` 只携带服务端公开的 `handRank.bestFiveCards`，牌桌仅以该字段让七张候选牌中的非最佳牌淡出并组合五张，不计算牌型、赢家或补牌。
