...
- `token-store.ts`：Token 仅内存与按 roomId 隔离的 `sessionStorage`；同 Tab 的非敏感 playerId 用于将投影识别为当前用户。二者都不使用 localStorage、IndexedDB、URL 或日志，storage 抛错时安全降级到内存。TEX-34 / TEX-50 增加 `clearIfMatches(roomId, token, reason)` 原子安全清理，严格校验当前持有凭据一致性，防止迟到错误响应误清已建立的新会话。
