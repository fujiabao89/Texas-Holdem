...
TEX-26 的 `animations/` 和 `audio/` 仍是跨功能 presentation 能力：前者只观察连续 Event 已提交后的 canonical target，并将公开 Board 和服务端 `bestFiveCards` 编排为入框/翻面/组合视觉；后者只播放预加载、本地 CC0 素材并复用短音效 voice；二者均不能发送 Action、推导规则或建立网络连接。
