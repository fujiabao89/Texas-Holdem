...
TEX-51：恢复注册返回可等待的启动结果，进行中比赛只有所有断线初始化与 START 成功后才解除屏障；失败撤销本次执行器和所有计时器。TEX-54 扩展 `createRecovered`：终局直接创建 FINISHED 只读 Runtime、保留已提交手 ID/序列并安排有界卸载，不投递 START/断线初始化或重复终局通知，不占活跃名额。相同 tournamentId 不允许覆盖旧执行器。完整恢复由 `persistence/room-recovery.ts` 先验证 Room 与身份，不再单独暴露无 Room 的比赛。
...
| 文件 | 职责 |
| --- | --- |
| `tournament-commands.ts` | Tournament 串行队列命令联合类型（Action / Time Bank / 行动与展示 Timer 回调 / 撤回 / 连接变化 / 升盲 / 关停）。`receivedAt`/`ingressOrdinal` 是服务端入口元数据，不是 wire 字段。 |
...
| `tournament-executor.ts` | **核心**：唯一串行执行器。真队列 + 截止点 look-ahead（§7.2）；幂等/sequence/身份/Engine 校验（§7.3）；权威展示阶段 / Action Timer / Time Bank / 断线宽限 / 定时升盲（§8）；无真人关房（§6.5）；撤回流程（§6.6）；手末 Commit Bundle（§12）；`PAUSE_AFTER_HAND` 背压手间边界（§12.2）。 |
