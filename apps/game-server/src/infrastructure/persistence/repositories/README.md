...
- **终局房间状态幂等重申**：非 CLOSED 的 `roomStatus` 更新带 `rooms.status = "FINISHED"` 守卫——延迟落库的终局 Bundle（PersistenceWriter 积压/重试）不得覆写已推进的房间（房主已"再来一局"写回 IN_GAME、或末位真人离开已 CLOSED）；房间为 FINISHED 是控制面 `TOURNAMENT_FINISHED` 先提交后确认的必然结果，守卫命中时为幂等 no-op。
