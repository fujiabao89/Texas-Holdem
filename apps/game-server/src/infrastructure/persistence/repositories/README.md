...
- **终局房间状态守卫**：非 CLOSED 的 `roomStatus` 更新带条件守卫——房间 `FINISHED`（控制面 `TOURNAMENT_FINISHED` 先提交后确认的正常时序，幂等重申）或房间 `IN_GAME` 且同房间无其他运行中比赛（控制面瞬时失败被 main.ts 丢弃后由本 Bundle 兜底补写）时写入；房间 `IN_GAME` 且新赛已运行（再来一局竞态）或 `CLOSED`/`LOBBY` 时跳过，延迟落库的终局 Bundle（PersistenceWriter 积压/重试）不得覆写已推进的房间状态。
