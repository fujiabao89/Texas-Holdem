...
- **串行执行**：`room-executor.ts` 每个 Room 一个串行队列；HTTP/WS 只能经 `RoomManager.submitCommand` 投递命令，不得直接 mutate。控制面先提交（先持久化成功再确认），避免半提交与检查后写入竞态；WS 的 Ready/离开命令在取得队列执行权时复核连接 epoch。
