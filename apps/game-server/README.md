...
- `src/tournaments/` — Tournament 运行时、单桌串行执行器（`receivedAt` 截止裁决、幂等/sequence、TEX-59 展示阶段后开放完整行动时钟）、手末 Commit Bundle 构造与崩溃恢复运行时重建（TEX-20/TEX-22），见 [src/tournaments/README.md](./src/tournaments/README.md)。
...
TEX-53 将权威 D/SB/BB 同步到完整 Snapshot 与逐事件 patch，wire v4 要求前后端同步发布，版本边界见 [ADR-0005](../../docs/adr/0005-tex-53-authoritative-blind-seats.md)。持久化格式与引擎规则不变。
