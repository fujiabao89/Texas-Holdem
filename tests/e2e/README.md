...
- 业务场景（创建房间、下注、重连等）按目录组织：[all-in/](./all-in)、[betting/](./betting)、[create-room/](./create-room)、[full-hand/](./full-hand)、[join-table/](./join-table)、[reconnect/](./reconnect)、[side-pot/](./side-pot)、[table-layout/](./table-layout)。TEX-25 的 [betting/table.spec.ts](./betting/table.spec.ts) 通过 Playwright WebSocket mock 注入合法投影，覆盖键盘跟注、All-in 两步、房间关闭、被移出与 Session Replaced；TEX-26 的 [reconnect/tex-26.spec.ts](./reconnect/tex-26.spec.ts) 仅覆盖新增的接管对话框可访问性。两者均不依赖真实 game-server 或 sleep。
