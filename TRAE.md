...
开发完成后的 DeepSeek Harness 审查由用户手动启动。Trae Work 不得自行调用 [DEEPSEEKHARNESS_REVIEW_AGENT_PROMPT.md](./DEEPSEEKHARNESS_REVIEW_AGENT_PROMPT.md) 所定义的审查 Agent，也不得把该审查作为 `git add`、`git commit`、`git push` 或创建 PR 的前置门禁。
